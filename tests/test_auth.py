...
- token：签发/读取、过期、篡改
- HTTP 权限矩阵：未登录 401；学生放行 vocabulary/成绩读写等学习端点；教师全通
"""
import pytest
...
def client(users_file, tmp_path, monkeypatch):
    # 隔离成绩库到 tmp：默认路径探测会命中真实 iCloud exam.db，既有数据会干扰断言
...
def test_student_scope(client):
    ...
    # GET /exams 已放行（成绩簿是家庭单账户库，学生看到的就是自己的成绩）：
    # 隔离库为空起步，列表里应恰有刚提交的这一条
    r_list = client.get("/exams", headers=h)
    assert r_list.status_code == 200
    rows = r_list.json()["data"]
    assert [(x["id"], x["score"]) for x in rows] == [(exam_id, 90)], rows
    # 学生禁用：改动/删除成绩（管理）、课程讲次、知识点检索
